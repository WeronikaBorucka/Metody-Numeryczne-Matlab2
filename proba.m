...
%wykres wielomianu interpoluj¹cego (linia zielona)
figure
plot(w1,W,'r-')
title('Interpolacja punktow wielomianem')
hold on;

%wykres punktow ktore byly interpolowane (dla x, czarne punkty)
plot(x,y,'kx')
...
RF_x = vpa(falsi(f, min_x0 - 0.1, min_x0 + 0.1, dokl, max_iter),17);
 
S_x = vpa(sieczne(f, mid_x0 - 0.1, mid_x0 + 0.1, dokl, max_iter),17);
 
N_x = vpa(Newton(f, f_poch, max_x0 + 0.1, dokl, max_iter),17);
 ...
 for i= 1:size(dokl,2)
     ...
     x_zab(i,1) = vpa(falsi(f, min_x0 - 0.1, min_x0 + 0.1, dokl(i), max_iter),17);
     x_zab(i,2) = vpa(sieczne(f, mid_x0 - 0.1, mid_x0 + 0.1, dokl(i), max_iter),17);
     x_zab(i,3) = vpa(Newton(f, f_poch, max_x0 + 0.1, dokl(i), max_iter),17);
 ...
 figure
 loglog(dokl,(x_zab(:,4))','b-', dokl,(x_zab(:,5))','r--', dokl,(x_zab(:,6))','k-.')
...
max_iter = 10000 ;
 ...
 el = zeros(size(dokl,2),3);
  ...
  for i= 1:size(dokl,2)
     ...
     for j = 1:100 
         ...
         start = tic;
         vpa(falsi(f, min_x0 - 0.05, min_x0 + 0.05, dokl(i), max_iter),17);
         el(i,1) = el(i,1) + toc(start);

         start2 = tic;
         vpa(sieczne(f, mid_x0 - 0.05, mid_x0 + 0.05, dokl(i), max_iter),17);
         el(i,2) = el(i,2) + toc(start2);

         start3 = tic;
         vpa(Newton(f, f_poch, max_x0 + 0.05, dokl(i), max_iter),17);
 ...
 for i= 1:size(odl,2)
     ...
     start = tic;
     x_z(i,1) = abs( vpa(falsi(f, min_x0 - odl(i), min_x0 + odl(i), dokl, max_iter),17) - min_x0);
     el(i,1) = el(i,1) + toc(start);
     
     start2 = tic;
     x_z(i,2) = abs( vpa(sieczne(f, mid_x0 - odl(i), mid_x0 + odl(i), dokl, max_iter),17) - mid_x0);
     el(i,2) = el(i,2) + toc(start2);
     
     start3 = tic;
     x_z(i,3) = abs( vpa(Newton(f, f_poch, max_x0 + odl(i), dokl, max_iter),17) - max_x0);
     el(i,3) = el(i,3) + toc(start3); 
